...
parser = argparse.ArgumentParser(
    "Segmentation efficiency eavluation for docker containers", add_help=False
)
parser.add_argument(
    "-i",
    "--test_img_path",
...
parser.add_argument(
    "-o",
    "--save_path",
    default="./val_team_seg",
    type=str,
    help="segmentation output path",
)
parser.add_argument(
    "-d",
    "--docker_folder_path",
...
for docker in dockers:
    try:
        ...
        for case in test_cases:
            ...
            cmd = 'docker container run --gpus="device=0" -m 28g --name {} --rm -v $PWD/inputs/:/workspace/inputs/ -v $PWD/outputs/:/workspace/outputs/ {}:latest /bin/bash -c "sh predict.sh" '.format(
                teamname, teamname
            )
            ...
            if pix_num <= 1000000:
                ...
                rank_running_time = np.max(
                    [0, real_running_time - 10 * (pix_num / 1000000)]
                )
            ...
            try:
                ...
                print(
                    "Wrong segmentation name!!! It should be image_name.split('.')[0] + '_label.tiff' "
                )
        metric_df = pd.DataFrame(metric)
        metric_df.to_csv(
            join(team_outpath, teamname + "_running_time.csv"), index=False
        )
